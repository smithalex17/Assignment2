...
public class Graph {
    ...
    public int findComponents() {
        ...
        for(Stop s : stopList){
            visitAllConnections(s, visitOrder);
        }
        ...
        for(int i = visitOrder.size()-1; i <= 0; i--){
            assignRoot(visitOrder.get(i), visitOrder.get(i+1), components);
        }       
    ...
    public void assignRoot(Stop stop, Stop root, int component) {
        ...
        for(Edge e  : stop.getNeighbours()){
            if(e.getToStop().getRoot() == null){
                assignRoot(e.getToStop(), stop, component);
            }
        }
    ...
    public void addWalkingEdges(double walkingDistance) {
        ...
        for(Stop stopFrom : stopList){
            for(Stop stopTo : stopList){
                if(stopFrom.getId() != stopTo.getId()){
                    ...
                    double dis = stopFrom.distance(stopTo);
                    if(dis < walkingDistance){
                        stopFrom.addNeighbour( new Edge(stopFrom, stopTo, Transport.WALKING_TRIP_ID, stopFrom.distance(stopTo)/Transport.WALKING_SPEED_MPS));
                        count++;
                    }
                }
